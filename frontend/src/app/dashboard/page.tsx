    ...
    <div className="p-6 md:p-12 lg:p-16 max-w-[1400px] mx-auto w-full">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <Link
            href="/trace"
            className="flex-1 md:flex-none border border-border-subtle px-5 py-2.5 rounded-lg font-semibold text-sm transition-colors flex items-center gap-2"
          ...
          <button className="flex-1 md:flex-none text-ink px-5 py-2.5 rounded-lg font-semibold text-sm transition-transform active:scale-95"
            style={{ background: "linear-gradient(135deg, #9A7A10, #C9A227, #F0D060)", boxShadow: "0 2px 16px rgba(201,162,39,0.3)" }}>
            Escalate Response
          </button>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 block md:grid-cols-12 gap-6">
        ...
        <div className="col-span-1 md:col-span-7 bg-card border border-border-subtle rounded-2xl p-8 shadow-warm flex flex-col justify-between">
          ...
          <div className="flex gap-3 flex-wrap mt-8">
            <div className="px-3 py-2 rounded-full font-bold text-xs" style={{ background: "rgba(201,162,39,0.1)", color: "#9A7A10", border: "1px solid rgba(201,162,39,0.3)" }}>C2 Active</div>
            <div className="px-3 py-2 rounded-full font-bold text-xs" style={{ background: "rgba(201,162,39,0.1)", color: "#9A7A10", border: "1px solid rgba(201,162,39,0.3)" }}>Persistence Attempted</div>
            <div className="px-3 py-2 rounded-full font-bold text-xs" style={{ background: "rgba(201,162,39,0.1)", color: "#9A7A10", border: "1px solid rgba(201,162,39,0.3)" }}>Credential Access</div>
          </div>
        </div>
        ...
        <div className="col-span-1 md:col-span-5 bg-card border border-border-subtle rounded-2xl p-8 shadow-warm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-8">
            <p className="text-xs tracking-widest uppercase text-muted font-bold flex items-center gap-2">
              <Cpu className="w-4 h-4" style={{ color: "#C9A227" }} /> Active Agents
            </p>
            <span className="text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1.5"
              style={{ border: "1px solid rgba(201,162,39,0.35)", background: "rgba(201,162,39,0.08)", color: "#C9A227" }}>
          ...
          <div className="flex gap-3 mt-8">
            <input 
              ...
              className="flex-1 border rounded-lg px-4 py-2 text-sm bg-white focus:outline-none transition-shadow"
        ...
        <div className="col-span-1 md:col-span-7 bg-card border border-border-subtle rounded-2xl p-8 shadow-warm">
          <div className="flex items-center justify-between mb-8">
            <p className="text-xs tracking-widest uppercase text-muted font-bold flex items-center gap-2">
              <Zap className="w-4 h-4" style={{ color: "#C9A227" }} /> Agentic Activity Feed
            </p>
          ...
          <div className="relative border-l border-border-subtle ml-3 space-y-8 pb-4">
            {activity.map((item, idx) => (
              <div key={idx} className="relative pl-6">
                <div className={`absolute -left-[5px] top-1 w-2.5 h-2.5 rounded-full ring-4 ring-card ${idx === activity.length - 1 ? 'animate-pulse' : 'bg-muted/40'}`}
                  style={idx === activity.length - 1 ? { background: "#C9A227" } : {}}></div>
        ...
        <div className="col-span-1 md:col-span-5 bg-card border border-border-subtle rounded-2xl p-8 shadow-warm flex flex-col">
          ...
          <div className="flex-1 bg-white border border-border-subtle rounded-xl p-6 min-h-[220px] relative flex flex-col justify-end overflow-hidden">
             ...
             <svg className="absolute inset-0 w-full h-full object-cover opacity-80" viewBox="0 0 520 240" role="img" aria-label="Detonation graph">
              <path d="M60 120 C140 40, 220 40, 300 110" fill="none" stroke="rgba(201,162,39,0.45)" strokeWidth="3" strokeDasharray="4 4"/>
              <path d="M300 110 C360 160, 430 170, 480 120" fill="none" stroke="rgba(139,0,0,0.45)" strokeWidth="3" />
              <circle cx="60" cy="120" r="14" fill="#C9A227" opacity="0.3"/>
              <circle cx="190" cy="70" r="10" fill="#C9A227" opacity="0.65" />
              <circle cx="300" cy="110" r="16" fill="#8B0000" opacity="0.7" />
              <circle cx="410" cy="150" r="10" fill="#C9A227" opacity="0.6" />
              <circle cx="480" cy="120" r="12" fill="#8B0000" opacity="0.7" />

              <text x="52" y="155" fill="#9A7A10" fontSize="12" fontFamily="var(--font-mono)">Ingress</text>
              <text x="165" y="45" fill="#C9A227" fontSize="12" fontFamily="var(--font-mono)">Decode</text>
              <text x="275" y="145" fill="#8B0000" fontSize="14" fontWeight="bold" fontFamily="var(--font-mono)">Registry</text>
              <text x="388" y="185" fill="#C9A227" fontSize="12" fontFamily="var(--font-mono)">Dropper</text>
              <text x="455" y="95" fill="#8B0000" fontSize="12" fontFamily="var(--font-mono)">Beacon</text>
            </svg>
          </div>
          ...
          <div className="flex gap-6 mt-6 pt-6 border-t border-border-subtle">
            <div className="flex items-center gap-2 text-xs font-semibold text-muted tracking-wider uppercase">
              <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#C9A227" }}></span> Execution pivot
            </div>
            <div className="flex items-center gap-2 text-xs font-semibold text-muted tracking-wider uppercase">
        ...
        <div className="col-span-1 md:col-span-12 bg-card border border-border-subtle rounded-2xl p-8 shadow-warm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            ...
            <div>
              ...
              <ul className="space-y-3 font-mono text-sm text-ink/80">
                <li className="flex items-center gap-2"><span style={{ color: "#C9A227" }}>●</span> C:\ProgramData\System\wmi.dll</li>
                <li className="flex items-center gap-2"><span style={{ color: "#C9A227" }}>●</span> C:\Users\Public\svc.exe</li>
              </ul>
            </div>
      ...
      {/* Floating voice chat — renders as a fixed overlay */}
      <VoiceChat />
    </div>
  );
}
